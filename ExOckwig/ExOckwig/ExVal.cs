    ...
    internal class ExVal //Expression Evaluation
    {
        ...
        ********************************************************************/
        public ExVal(string inputFile, out LinkedList<Literal> LiteralTable, BST SymbolTable)
        {
            ...
            if (inputFile == null)
            {
                Console.Write("Please enter the expressions file to read: ");
                inputFile = Console.ReadLine();
            }
            try
            {
                string relative = "ExOckwig" + Path.DirectorySeparatorChar + inputFile;
            
                lineValues = File.ReadAllLines(relative);
            }
            catch (Exception ex)
            {
            ...
            Console.WriteLine("Expression     Value     RELOCATABLE   N-Bit   I-Bit   X-Bit");
            for (int j = 0; j < lineValues.Length; j++)
            {
                string lineValue = lineValues[j];
                lineValue = lineValue.Trim();
                string outputExpress = lineValue;
                string[] Symbols = { };
                int ExpValue = 0;
                string value = "";
                ...
                if (lineValue.Length > 2)
                {
                    ...
                    {
                        IndexBit = true;
                        lineValue = lineValue.TrimEnd('X');
                        lineValue = lineValue.TrimEnd(',');
                    }
                }
                switch (CheckLine(lineValue))
                {
                    case 1://literal

                        Error = true;
                        char[] charLit = lineValue.ToCharArray();
                        ...
                        if (LitType == 'C' || LitType == 'c')
                        {
                            ...
                            Literal Lit = new Literal(lineValue, value, size, LiteralTable.Count, LitType);
                            LiteralTable.AddLast(Lit); //adds if doesn't exist

                        }
                        else if (LitType == 'X' || LitType == 'x')
                        {
                            if (lineValue.Length % 2 == 1)
                            {
                                value = "0";
                            }
                            for (int i = 3; i < charLit.Length - 1; i++)
